    ...
    def train(it):
        ...
        for _ in range(config.train.n_acc_batch):
            ...
            if np.random.random() < 0.4:
                is_in_pocket = torch.zeros([batch.protein_element_batch.size(0)],
                                           device=batch.protein_element_batch.device)
            else:
            ...
            loss, loss_pos, loss_v, loss_pos_motif, loss_v_motif, loss_ph = results['loss'], results['loss_pos'], \
                                                                            results['loss_v'], results[
                                                                                'loss_pos_motif'], results[
                                                                                'loss_v_motif'], results['loss_ph']
        ...
        if it % args.train_report == 0:
            logger.info(
                '[Train] Iter %d | Loss %.6f (pos %.6f | v %.6f | pos_m %.6f | v_m %.6f | ph %.6f)  | Lr: %.6f | Grad Norm: %.6f' % (
                    it, loss, loss_pos, loss_v, loss_pos_motif, loss_v_motif, loss_ph, optimizer.param_groups[0]['lr'],
                    orig_grad_norm
                )
            )
    ...
    def validate(it):
        ...
        with torch.no_grad():
            ...
            for batch in tqdm(val_loader, desc='Validate'):
                ...
                for t in np.linspace(0, model.num_timesteps - 1, 10).astype(int):
                    ...
                    loss, loss_pos, loss_pos_motif, loss_v, loss_v_motif, loss_ph = results['loss'], results[
                        'loss_pos'], results['loss_pos_motif'], results['loss_v'], results['loss_v_motif'], results[
                                                                                        'loss_ph']
        ...
        logger.info(
            '[Validate] Iter %05d | Loss %.6f | Loss pos %.6f |  Loss pos motif %.6f | Loss v %.6f e-3 | Loss v motif %.6f e-3 | Loss ph %.6f | Avg atom auroc %.6f' % (
                it, avg_loss, avg_loss_pos, avg_loss_pos_motif, avg_loss_v * 1000, avg_loss_v_motif * 1000, avg_loss_ph,
                atom_auroc
            )
        )
