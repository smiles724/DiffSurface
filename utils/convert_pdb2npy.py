...
def load_seperate_structure(fname, return_map=False, ligand=None, receptor=None):
    ...
    return {'ligand': {"atom_xyz": atom_coords_ligand, "atom_types": atom_types_ligand_array, 'res_xyz': res_coords_ligand, 'res_types': res_types_ligand, 'seq_map': seq_map},
...
def load_structure(fname, return_map=False):
    ...
    atom_coords, atom_types, res_coords, res_types = [], [], [], []
    chain_ids, resseqs, icodes = [], [], []
    for i, res in enumerate(residues):
        resname = res.get_resname()
        if AA.is_aa(resname):        # skip unknown residues
    ...
    if return_map:
        for i, (chain_id, resseq, icode) in enumerate(zip(chain_ids, resseqs, icodes)):
            if (chain_id, resseq, icode) in seq_map.keys():
                ...
                continue
            seq_map[(chain_id, resseq, icode)] = i
    return {"atom_xyz": atom_coords, "atom_types": atom_types_array, 'res_xyz': res_coords, 'res_types': res_types, 'seq_map': seq_map}
