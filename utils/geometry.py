...
def soft_distances(x, y, batch_x, batch_y, smoothness=0.01, atomtypes=None):
    ...
    if atomtypes is not None:
        ...
        try:
            ...
            atomic_radii = torch.FloatTensor([170, 110, 152, 155, 180, 190], device=x.device)   # support cpu
...
def atoms_to_points(atoms, batch, distance=1.05, smoothness=0.5, resolution=1.0, nits=4, atomtypes=None, sup_sampling=20, variance=0.1, ):
    ...
    Returns:
        (Tensor): (M,3) coordinates for the structures points `x_i`.
...
def mesh_normals_areas(vertices, triangles=None, scale=[1.0], batch=None, normals=None):
    ...
    Simply put - if `triangles` are provided:
      1. Normals are first computed for every triangle using simple 3D geometry
         and are weighted according to structures area.
    ...
    if triangles is not None:
        ...
        # Triangle centers and normals (length = structures area):
        centers = (A + B + C) / 3  # (N, 3)
        V = (B - A).cross(C - A)  # (N, 3)
...
def curvatures(vertices, triangles=None, scales=[1.0], batch=None, normals=None, reg=0.01):
    ...
    We rely on a very simple linear regression method, for all vertices:
      ...
      1. Estimate normals and structures areas.
    ...
    As of today, this implementation does not weigh points by structures areas:
    ...
    normals_s, _ = mesh_normals_areas(vertices, triangles=triangles, normals=normals, scale=scales, batch=batch)  # (N, S, 3), (N,)
...
class dMaSIFConv(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, radius=1.0, hidden_units=None, cheap=False):
        ...
        if self.Hidden % self.heads_dim != 0:
            raise ValueError(f"The dimension of the hidden units ({self.Hidden})" + f"should be a multiple of the heads dimension ({self.heads_dim}).")
        else:
            self.n_heads = self.Hidden // self.heads_dim

        # Transformation of the input features:
        self.net_in = nn.Sequential(nn.Linear(self.Input, self.Hidden),  # (H, I) + (H,)
            nn.LeakyReLU(negative_slope=0.2), nn.Linear(self.Hidden, self.Hidden),  # (H, H) + (H,)
            # nn.LayerNorm(xxx.Hidden),#nn.BatchNorm1d(xxx.Hidden),
            nn.LeakyReLU(negative_slope=0.2), )  #  (H,)
        self.norm_in = nn.GroupNorm(4, self.Hidden)
        # xxx.norm_in = nn.LayerNorm(xxx.Hidden)
        # xxx.norm_in = nn.Identity()

        # 3D convolution filters, encoded as an MLP:
        if cheap:
            self.conv = nn.Sequential(nn.Linear(3, self.Hidden), nn.ReLU()  # (H, 3) + (H,)
            )  # KeOps does not support well LeakyReLu
        else:
            self.conv = nn.Sequential(nn.Linear(3, self.Cuts),  # (C, 3) + (C,)
                nn.ReLU(),  # KeOps does not support well LeakyReLu
                nn.Linear(self.Cuts, self.Hidden), )  # (H, C) + (H,)

        # Transformation of the output features:
        self.net_out = nn.Sequential(nn.Linear(self.Hidden, self.Output),  # (O, H) + (O,)
            nn.LeakyReLU(negative_slope=0.2), nn.Linear(self.Output, self.Output),  # (O, O) + (O,)
            # nn.LayerNorm(xxx.Output),#nn.BatchNorm1d(xxx.Output),
            nn.LeakyReLU(negative_slope=0.2), )  #  (O,)

        self.norm_out = nn.GroupNorm(4, self.Output)
        # xxx.norm_out = nn.LayerNorm(xxx.Output)
        # xxx.norm_out = nn.Identity()
        ...
        with torch.no_grad():
            ...
            if not cheap:
                nn.init.uniform_(self.conv[2].weight, a=-1 / np.sqrt(self.Cuts), b=1 / np.sqrt(self.Cuts), )
    ...
    def forward(self, points, nuv, features, ranges=None):
        ...
        normals = (nuv[:, 0, :].contiguous().detach())  # (N, 3) - remove the .detach() if needed
        ...
        # over the vector of length "xxx.Hidden = xxx.n_heads * xxx.heads_dim"
        # as xxx.n_heads reduction over vectors of length xxx.heads_dim (= "Hd" in the comments).
        head_out_features = []
        for head in range(self.n_heads):
            ...
            # MLP:
            if self.cheap:
                X_ij = ab.matvecmult(X_ij.concat(LazyTensor(1)))  # (N, N, Hd*4) @ (N, N, 3+1) = (N, N, Hd)
